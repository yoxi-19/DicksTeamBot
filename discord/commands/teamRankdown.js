// Variable: /team rankdown - Stuft ein Team-Mitglied einen Rang runter (z.B. Team 4 -> Team 5).
...
export default {
  data: new SlashCommandBuilder()
    .setName('team-rankdown')
    .setDescription('Stuft ein Team-Mitglied einen Rang runter (z.B. Team 4 -> Team 5).')
  ...
  async execute(interaction) {
    ...
    if (oldRank >= cfg.count) {
      await interaction.reply({
        embeds: [errorEmbed('Bereits unten', `**${found.user.ign}** ist bereits in Team **${cfg.count}** (niedrigster Rang).`)],
        ephemeral: true,
      });
      return;
    }
    ...
    await interaction.reply({
      embeds: [successEmbed('Rank down', `**${found.user.ign}** wurde von Team **${result.oldRank}** auf Team **${result.newRank}** runtergestuft.`)],
      ephemeral: true,
    });

    logger.info(`[Discord] Admin ${interaction.user.tag} hat ${found.user.ign} auf Team ${result.newRank} runtergestuft.`);
  },
};
